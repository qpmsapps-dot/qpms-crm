-- Read-only pre-deployment verification for Migration 103.
-- Expected Supabase project: ubawkjdbtickdlrgkulz
-- Expected NIMS client: bfb5d707-1a4e-451d-af1f-11b7c0aeeb66
...
where n.nspname = 'public'
  and p.proname = 'hospital_can_access_ticket';
...
select
  ...
  ) as operational_acceptance_rpc,
  to_regprocedure(
    'public.rpc_accept_hospital_supervisor_ticket(uuid,uuid,integer,boolean,timestamptz)'
  ) as legacy_supervisor_acceptance_rpc;

with nims_tickets as (
...
select count(*) as nims_event_count
...
select count(*) as nims_notification_count
