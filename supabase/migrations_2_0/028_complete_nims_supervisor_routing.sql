-- Phase 2C recovery: complete NIMS supervisor routing after partial 027 apply.
-- Forward-only and idempotent for the observed cloud state. Do not edit 027.
...
create table if not exists public.hospital_shifts (
  ...
  constraint hospital_shifts_days_check
    check (array_length(days_of_week, 1) is not null and days_of_week <@ array[0,1,2,3,4,5,6]::smallint[])
);
...
begin
  ...
  if coalesce(new.metadata->>'assignment_history_recorded','') = 'create_rpc' then
    return null;
  end if;
  v_source := case
...
begin
  ...
  insert into public.hospital_tickets (
    ...
    jsonb_build_object(
      ...
      'assignment_history_recorded', 'create_rpc',
