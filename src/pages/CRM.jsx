...
import LeadForm, {
  ButtonContent,
  LeadContactEditor as ContactPersonsEditor,
  FormSection,
  SelectField,
  ServiceScopeSelector,
  TextField,
} from '../components/preSales/LeadForm.jsx';
import {
  formatServiceScope,
  industryOptions,
  initialLeadForm,
...
const statusOptions = ['Active', 'Pending', 'Escalated', 'Completed', 'Qualified', 'Invalid', 'Converted', 'Archived', 'Lost'];
...
export default function CRM() {
  ...
  const isPreSalesWorkspace = searchParams.get('workspace') === 'pre-sales';
  const shouldOpenLeadForm = isPreSalesWorkspace && searchParams.get('action') === 'add';
  ...
  const [isFormOpen, setIsFormOpen] = useState(shouldOpenLeadForm && canCreateLeads);
  ...
  usePageTitle(isPreSalesWorkspace ? 'Pre-Sales Lead Intake' : 'Lead Management');
  ...
  useEffect(() => {
    if (!canAssignLeads) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- discard stale assignee options when access changes
      setLiveAssignees([]);
      return undefined;
    }
    let active = true;
  ...
  return (
    <div className="space-y-7">
      <PageHeader
        title={isPreSalesWorkspace ? 'Pre-Sales Lead Intake' : 'Lead Management'}
          ...
          <div className="modal-surface max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-3xl border border-white/70 bg-white p-5 shadow-[0_30px_100px_rgba(15,23,42,0.28)] dark:border-slate-800 dark:bg-slate-900 sm:p-6" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mt-6">
              <LeadForm
                ...
                submitting={pendingAction === 'createLead'}
                submitLabel="Create Lead"
                assignmentLabel="Assign to BD Executive"
                assignmentKey="assigned_bd_profile_id"
                assignmentOptions={liveAssignees.map((assignee) => ({
                  value: assignee.id,
                  label: `${assignee.full_name} — ${assignee.employee_code}`,
                }))}
                assignmentEnabled={canAssignLeads}
                selfAssignmentMessage={isBdExecutive ? 'This lead will be assigned to you.' : ''}
              />
            </div>
          </div>
        </div>
