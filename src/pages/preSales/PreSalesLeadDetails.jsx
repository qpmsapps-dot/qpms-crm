...
const tabs = ['Overview', 'Call History', 'Follow-ups', 'Meetings & MOM', 'Documents', 'Handover'];
...
export default function PreSalesLeadDetails() {
  ...
  useEffect(() => { if (activeTab !== 'Overview' && activeTab !== 'Documents') void loadTab(activeTab); }, [activeTab, loadTab]);
...
function TabContent({ tab, lead, state, reload, setNotice, onCompleteFollowup, onRescheduleFollowup, onCreateMeeting, onCompleteMeeting, onCreateHandoff }) {
  ...
  if (tab === 'Follow-ups') return <Followups items={state.items} onComplete={onCompleteFollowup} onReschedule={onRescheduleFollowup} />;
  if (tab === 'Meetings & MOM') return <Meetings items={state.items} leadMom={lead.lead_mom || []} onCreate={onCreateMeeting} onComplete={onCompleteMeeting} setNotice={setNotice} />;
  if (tab === 'Handover') return <Handover items={state.items} assignees={state.assignees} lead={lead} onCreate={onCreateHandoff} />;
  return null;
}
...
function Followups({ items, onComplete, onReschedule }) {
  const { user } = useAuth();
  const editable = canEditPreSalesLead(user);
...
function Meetings({ items, leadMom, onCreate, onComplete }) {
  const { user } = useAuth();
  const editable = canEditPreSalesLead(user);
...
function Handover({ items, assignees = [], lead, onCreate }) {
  const { user } = useAuth();
  const editable = canEditPreSalesLead(user);
