...
export function buildEmployeeRangeExcelRows(dataset) {
  ...
  return {
    periodSummary: [{
      ...
      "Current Payable KM": summary.canonical_payable_km ?? summary.kilometer ?? 0,
      "Pending Missing KM": summary.pending_missing_km ?? 0,
      "Approved Missing KM": summary.approved_missing_km ?? 0,
      "Final Payable KM": summary.canonical_payable_km ?? summary.kilometer ?? 0,
      "Distance Reimbursement": summary.distance_amount ?? 0,
      "Pending Missing KM Amount": summary.pending_missing_km_amount ?? 0,
      "Approved Missing KM Amount": summary.approved_missing_km_amount ?? 0,
    ...
    siteVisits: (dataset?.site_visits || dataset?.site_visit_summary || []).map((row) => {
      const review = row?.metadata?.missing_km_review || {};
      return ({
      ...
      "Checkout Deviation": row.checkout_distance_meters ?? "",
      "Detected Missing KM": review.detected_missing_km ?? "",
      "Already Included KM": review.already_included_km ?? "Requires Review",
      "Approval Missing KM": review.suggested_missing_km ?? 0,
      "Status": review.status || row?.metadata?.checkout_review_status || "",
      "Approved KM": review.approved_missing_km ?? 0,
      "Rate": review.rate_per_km ?? "",
      "Approved Amount": review.approved_amount ?? 0,
      "Reason": review.reason_code || "",
      "Calculation Source": review.calculation_source || "",
      "Reviewer": review.reviewer_name || review.reviewer_employee_code || "",
    });}),
