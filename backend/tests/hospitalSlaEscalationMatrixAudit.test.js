...
function enterLevel(ticket, users, fromLevel, now, reason) {
  ...
  const acceptanceDue = new Date(now + 2 * minute).toISOString();
...
function acceptCurrentLevel(ticket, actorId, now) {
  ...
  if (level === 3) ticket.escalation_due_at = new Date(now + 20 * minute).toISOString();
...
test('2-minute acceptance SLA scenarios pass deterministically without real waits', () => {
  const rows = [
    resultRow({
      scenario: 'Supervisor T+1:59',
      ticket: baseTicket({ acceptance_due_at: new Date(t0 + 119000).toISOString() }),
      timeOffset: 1.98,
      expectedNextRole: null,
    }),
    resultRow({
      scenario: 'Supervisor T+2:00',
      ticket: baseTicket(),
      timeOffset: 2,
    ...
    resultRow({
      scenario: 'Operations T+10:03:59',
      ticket: enterLevel(baseTicket(), defaultUsers, 1, t0 + 2 * minute, 'supervisor_acceptance_timeout'),
      timeOffset: 3.98,
      expectedNextRole: 'operations_executive',
    }),
    resultRow({
      scenario: 'Operations T+10:04',
      ticket: enterLevel(baseTicket(), defaultUsers, 1, t0 + 2 * minute, 'supervisor_acceptance_timeout'),
      timeOffset: 4,
      expectedNextRole: 'facility_manager',
    }),
    resultRow({
      scenario: 'Operations accepted T+10:03',
      ticket: enterLevel(baseTicket(), defaultUsers, 1, t0 + 2 * minute, 'supervisor_acceptance_timeout'),
      accepted: true,
      acceptAtOffset: 3,
      acceptUserId: 'ops-1',
    ...
    resultRow({
      scenario: 'Facility T+10:05:59',
      ticket: enterLevel(baseTicket(), defaultUsers, 2, t0 + 4 * minute, 'operations_executive_acceptance_timeout'),
      timeOffset: 5.98,
      expectedNextRole: 'facility_manager',
    }),
    resultRow({
      scenario: 'Facility T+10:06',
      ticket: enterLevel(baseTicket(), defaultUsers, 2, t0 + 4 * minute, 'operations_executive_acceptance_timeout'),
      timeOffset: 6,
      expectedNextRole: 'project_head',
    }),
    resultRow({
      scenario: 'Facility accepted before 10:06',
      ticket: enterLevel(baseTicket(), defaultUsers, 2, t0 + 4 * minute, 'operations_executive_acceptance_timeout'),
      accepted: true,
      acceptAtOffset: 5,
  ...
  assert.equal(rows[4].actualAcceptanceDueAt, iso(6));
  assert.equal(rows[7].actualAcceptanceDueAt, iso(8));
});

test('Project Head has no invented higher role and final timeout is one-shot', () => {
  const projectTicket = enterLevel(baseTicket(), defaultUsers, 3, t0 + 6 * minute, 'facility_manager_acceptance_timeout');
  processAcceptance(projectTicket, defaultUsers, t0 + 7.98 * minute);
  assert.equal(projectTicket.acceptance_status, 'awaiting');
  assert.equal(projectTicket.current_assignee_role, 'project_head');

  runTwice(projectTicket, defaultUsers, t0 + 8 * minute);
  assert.equal(projectTicket.current_assignee_role, 'project_head');
...
test('unavailable roles skip upward through the intended hierarchy', () => {
  ...
  assert.equal(noSupervisor.acceptance_due_at, iso(2));

  const noOps = defaultUsers.filter((user) => user.role_code !== 'operations_executive');
  const skippedOps = enterLevel(baseTicket(), noOps, 1, t0 + 2 * minute, 'supervisor_acceptance_timeout');
  ...
  assert.equal(skippedOps.acceptance_due_at, iso(4));

  const noFacility = defaultUsers.filter((user) => user.role_code !== 'facility_manager');
  const skippedFacility = enterLevel(baseTicket(), noFacility, 2, t0 + 4 * minute, 'operations_executive_acceptance_timeout');
  ...
  assert.equal(skippedFacility.acceptance_due_at, iso(6));
});
...
test('final statuses and duplicate processor runs are safe', () => {
  for (const status of ['resolved_awaiting_confirmation', 'closed', 'cancelled']) {
    const ticket = baseTicket({ status_code: status, acceptance_due_at: iso(2) });
  ...
  for (const [fromLevel, expectedRole, at] of [
    [1, 'operations_executive', 2],
    [2, 'facility_manager', 4],
    [3, 'project_head', 6],
  ]) {
